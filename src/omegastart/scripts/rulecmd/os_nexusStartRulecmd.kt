package data.omegastart.scripts.rulecmd
...
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.RARE_METALS_PER_NEXUS
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.SUPPLIES_PER_NEXUS
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.factionspec
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.rewardlist
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.ship
import data.omegastart.scripts.rulecmd.os_nexusStartRulecmd.Companion.targetmarket
...
class os_nexusStartRulecmd: BaseCommandPlugin() { // stuff to handle nexus interactions
    override fun execute(ruleId: String?, dialog: InteractionDialogAPI?, params: MutableList<Misc.Token>?, memoryMap: MutableMap<String, MemoryAPI>?): Boolean {
        ...
        when (type)  {
            ...
            0 -> {
                ...
                dialog.textPanel.addPara("The Coronal Hypershunt welcomes you into its graces, allowing you to make use of its services.")
           ...
           //     Global.getSettings().getWeaponSpec(it).tags.remove(Tags.RESTRICTED)}
                dialog!!.showCustomProductionPicker(os_nexusCustomProduction(dialog))
            }
            3 -> {
            ...
            dialog.optionPanel.addOption("Request immediate production", "os_nexusProductionPicker")
            dialog.optionPanel.setTooltip("os_nexusProductionPicker", "Instantly produce Omega hulls and weapons to be delivered to your fleet.")
            dialog.optionPanel.addOption("Access the storage network", "os_nexusStorage")
            dialog.optionPanel.setTooltip("os_nexusStorage", "Allows you to refit your ships. Counts as a spaceport for hullmods that require a dock.")
            ...
            7 -> {
                ...
                dialog.optionPanel.addOption("Use the Hypershunt to refit your ships", "marketOpenRefit")
            ...
            9 -> {
                ...
                dialog!!.textPanel.setFontSmallInsignia()
                dialog.optionPanel.clearOptions()
                dialog.textPanel.addPara("The ${ship!!.shipName} has been deconstructed. Moments later, a sonorous chime emits from the Hypershunt.")
                ...
                if (ship!!.hullSpec.baseHullId != "rat_genesis") {
                    ...
                    shipvarlist.forEach {
                        ...
                        Global.getSettings().addEntryForRole(Factions.OMEGA, role, it, (1f))
                    }
                }
                ...
                Global.getSector().allFactions.forEach {
                    if (!banlist.contains(it.id) && it.knowsShip(ship!!.hullSpec.baseHullId) && it.factionSpec.id != "omega") {
                        factionspec = it.id
                ...
                dialog.textPanel.addPara("Omega fleets may now use the ${ship!!.hullSpec.hullNameWithDashClass} ${ship!!.hullSpec.hullSize.name}.").setColor(Misc.getHighlightColor())


                    doFactionCheck(factionspec, hascores, dialog)

                dialog.textPanel.setFontInsignia()
                dialog.optionPanel.addOption("Continue", "os_nexusDeconstructMain")
            ...
            13 -> {
                dialog!!.textPanel.clear()
                dialog.textPanel.addPara("The Omega are ever-hungry to expand their knowledge, especially towards those whose hulls resemble their own.")
                dialog.xOffset = 0f
                if (height != null){
                    dialog.textHeight = height!!
                    dialog.textWidth = width!!
            ...
            14 -> { // call this to show all resource costs, then check resources and disable option to build nexus if either time is too early or
                val expire = Global.getSector().memoryWithoutUpdate.getExpire("\$os_nexusBuildTimeout")
                if  (Global.getSector().memoryWithoutUpdate.getExpire("\$os_nexusBuildTimeout") > 0f){
                    dialog!!.textPanel.addPara("It will take some time to prepare to construct another Hypershunt.")
                ...
                if (Global.getSector().memoryWithoutUpdate.getBoolean("\$os_nexusBuildTimeout") || !supplies || !metals || !raremetals){
                    dialog.optionPanel.setEnabled("os_nexusConstruct", false)
                    dialog.optionPanel.setTooltip("os_nexusConstruct", "Can't build this yet.")
                }
            }
            15 -> {
...
class os_nexusBuildScript(var source: CampaignFleetAPI, var loc: EntityLocation) : Script {
    override fun run() {
        ...
        Global.getSector().campaignUI.messageDisplay.addMessage("Hypershunt construction finished in ${system.name}")

//        val fleet = FleetFactoryV3.createEmptyFleet(Factions.OMEGA, FleetTypes.BATTLESTATION, null)
//
...
//        system.addScript(activeFleets)
        val spec = Global.getSettings().getCustomEntitySpec(Entities.CORONAL_TAP);
        val loc = EntityLocation()
        val star = system.star
        val orbitRadius = star.getRadius() + spec.getDefaultRadius() + 100f
...
private fun ShowNexusBuildPicker(dialog: InteractionDialogAPI){
    ...
    dialog.showCampaignEntityPicker("Title", "Selected", "Ok", Global.getSector().getFaction(Factions.OMEGA), centers, object : BaseCampaignEntityPickerListener(){
        ...
        override fun pickedEntity(entity: SectorEntityToken) {
            ...
            Global.getSector().memoryWithoutUpdate.set("\$os_nexusBuildTimeout", true, 90f)
            dialog.optionPanel.setEnabled("os_nexusConstruct", false)
            val constructorFleet = MagicCampaign.createFleetBuilder()
                .setFleetFaction(Factions.OMEGA)
            ...
            constructorFleet.addAssignment(FleetAssignment.ORBIT_PASSIVE, token, 30f, "Constructing Hypershunt", os_nexusBuildScript(constructorFleet, loc))
            dialog.textPanel.addPara("A fleet has been dispatched to ${system.name}.").setHighlight(system.name)
            dialog.textPanel.addPara("Once it arrives, it will take 30 days to construct the Hypershunt.").setHighlight("30 days")
        }
...
private fun getCargoPicker(dialog: InteractionDialogAPI?){
   ...
   val creds = Global.getSector().playerFleet.cargo.credits
    ...
    dialog!!.showCargoPickerDialog("Hypershunt Supply", "Requisition", "Cancel", true, 310f, cargo, object : CargoPickerListener {
        ...
        override fun recreateTextPanel(panel: TooltipMakerAPI?, cargo: CargoAPI, pickedUp: CargoStackAPI?, pickedUpFromSource: Boolean, combined: CargoAPI) {
            ...
            val hegsprite = Global.getSettings().getFactionSpec(Factions.OMEGA).crest
...
fun getCost(cargo: CargoAPI): Float {
    ...
    if (getValidShips().isEmpty()){
        dialog!!.textPanel.addPara("None of the automated ships in your fleet are unknown to the Omega.")
            ...
            object: FleetMemberPickerListener {
                override fun pickedFleetMembers(members: List<FleetMemberAPI?>?){
                    ...
                    for (member in members){
                        ...
                        dialog.optionPanel.addOption("Proceed with deconstruction", "os_nexusDeconstructProceed")
                        dialog.optionPanel.addOption("Go back", "os_nexusDeconstructMain")
                    }
...
fun getValidShips(): ArrayList<FleetMemberAPI> {
     val remmies = Global.getSector().getFaction(Factions.OMEGA)
...
class nexusStorageScript(
    ...
    private val storageEntity: SectorEntityToken? = Global.getSector().getEntityById("os_nexusStorage")
    ...
    if ((remmyships.size.toFloat() / facships.size.toFloat()) >= 0.5f && !mem.contains("\$os_"+ spec + "weapons")) {
        facweaps.forEach {
            remmy.addKnownWeapon(it, false)
        }
        Global.getSoundPlayer().playUISound("ui_char_spent_story_point", 1f,1f)
        dialog.textPanel.addPara("The Omega now know 50% or more of this faction's automated hulls.").setHighlight("50%")
        dialog.textPanel.addPara("Iterative analysis of their systems now allows the Omega to use all of this faction's known weapons.").setHighlight("known weapons.")
        ...
        mem.set("\$os_" + spec + "weapons", true)
    }
    if ((remmyships.size.toFloat() / facships.size.toFloat()) >= 0.75f && !mem.contains("\$os_"+ spec + "wings")) {
        facwings.forEach {
            remmy.addKnownFighter(it, false)
        }
        Global.getSoundPlayer().playUISound("ui_char_spent_story_point", 1f,1f)
        dialog.textPanel.addPara("The Omega now know 75% or more of this faction's automated hulls.").setHighlight("75%")
        dialog.textPanel.addPara("Comprehensive analysis of their systems now allows the Omega to use all of this faction's known fighters.").setHighlight("known fighters.")
        dialog.textPanel.addPara("These will also be available through production orders.")
        dialog.textPanel.setFontSmallInsignia()
        if (factionHasCores){
            dialog.textPanel.addPara("Coronal Hypershuntes will offer this faction's AI cores at 90%.").setColor(Misc.getHighlightColor())
        }
        mem.set("\$os_" + spec + "wings", true)
    }
    if ((remmyships.size.toFloat() / facships.size.toFloat()) >= 0.90f && !mem.contains("\$os_"+ spec + "cores") && factionHasCores) {
        Global.getSoundPlayer().playUISound("ui_char_spent_story_point", 1f,1f)
        dialog.textPanel.addPara("The Omega now know 90% or more of this faction's automated hulls.").setHighlight("90%")
        dialog.textPanel.addPara("Thorough reconstruction of their behaviors now allows any Hypershunt to offer some of the faction's AI cores.").setHighlight("AI cores.")
        dialog.textPanel.addPara("These are available through the supply cargo picker.")
        dialog.textPanel.setFontSmallInsignia()
        mem.set("\$os_" + spec + "cores", true)
    ...
    dialog.textPanel.addPara("The Omega know ${remmyships.size} of them.").setHighlight("${remmyships.size}")
}
fun showRecountInfo(dialog: InteractionDialogAPI){
    ...
    Global.getSector().allFactions.forEach { fac ->
        Global.getSettings().getFactionSpec(fac.id).knownShips.forEach {
            if (!factionlist.contains(fac.id) && !banlist.contains(fac.id) && Global.getSettings().getHullSpec(it).hasTag(Tags.AUTOMATED_RECOVERABLE) && Global.getSettings().getHullSpec(it).manufacturer!="Omega"){
    ...
    factionlist.forEach { facid ->
        ...
        img.addPara("The Omega currently know ${remmyships.size} of them.", 5f).setHighlight("${remmyships.size}")
...
fun doSetup(dialog: InteractionDialogAPI) {
    if (!Global.getSector().intelManager.hasIntelOfClass(os_nexusRaidIntel::class.java)) {
