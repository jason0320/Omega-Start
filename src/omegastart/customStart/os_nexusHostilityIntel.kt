...
class os_nexusHostilityIntel: BaseIntelPlugin(), EconomyTickListener {
    ...
    override fun advanceImpl(amount: Float) {
        if (!Global.getSector().listenerManager.hasListenerOfClass(os_nexusHostilityIntel::class.java)) {
            Global.getSector().listenerManager.addListener(os_nexusHostilityIntel(), true)
        }
    }
      ...
      //  }
            ...
            if (clock.getElapsedDaysSince(stamp!!) > days) {
                ...
                if (Misc.getDistance(loc, Vector2f(0f, 0f)) > 12000f && Misc.getDistance(loc, Vector2f(0f,0f)) < 35000f) {
                    if (Global.getSector().getFaction(Factions.INDEPENDENT
