...
class os_nexusRaidIntel (var bombmarket: MarketAPI, var reward: ArrayList<Int>): BaseIntelPlugin(), ColonyPlayerHostileActListener {
    val pcargo = Global.getSector().playerFleet.cargo
    val title = "Coordinated Raid"

    val memory = Global.getSector().memoryWithoutUpdate
    val key = "\$os_nexusParty"
    val sprite = Global.getSettings().getFactionSpec(Factions.OMEGA).crest
    ...
    override fun createIntelInfo(info: TooltipMakerAPI?, mode: IntelInfoPlugin.ListInfoMode?) {
        if (memory.getInt(key) == 1){
            info!!.addTitle(title)
            info.addPara("Return to any Hypershunt to receive the rewards.", 5f)
    ...
    override fun getFactionForUIColors(): FactionAPI {
        return Global.getSector().getFaction(Factions.OMEGA)
    }
    ...
    override fun reportRemovedIntel() {
        ...
        targetmarket = null
         rewardlist.clear()
         memory.set("\$os_nexusPartyTimeout", true, 180f)
        memory.set(key, 2)
