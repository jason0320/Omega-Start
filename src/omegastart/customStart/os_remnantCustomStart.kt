...
class os_remnantCustomStart: CustomStart() {
    ...
    override fun execute(dialog: InteractionDialogAPI?, memoryMap: MutableMap<String, MemoryAPI>?) {
        ...
        dialog.textPanel.addPara("Start under the Omega faction, being able to access any Hypershunt for normal fleet operations.")
        dialog.textPanel.addPara("As you are starting with a Omega commission, you will likely be hostile to most factions.")
        ...
        data.addStartingFleetMember("tesseract_Attack", FleetMemberType.SHIP)
        data.addStartingFleetMember("facet_Attack", FleetMemberType.SHIP)
        data.addStartingFleetMember("shard_left_Attack", FleetMemberType.SHIP)
        data.addStartingFleetMember("shard_left_Missile", FleetMemberType.SHIP)
        data.addStartingFleetMember("shard_right_Attack", FleetMemberType.SHIP)
        data.addStartingFleetMember("shard_right_Missile", FleetMemberType.SHIP)
        ...
        PlayerFactionStore.setPlayerFactionIdNGC(Factions.OMEGA)
        ...
        tempFleet.fleetData.apply {
            addFleetMember("tesseract_Attack")
            addFleetMember("facet_Attack")
            addFleetMember("shard_left_Attack")
            addFleetMember("shard_left_Missile")
            addFleetMember("shard_right_Attack")
            addFleetMember("shard_right_Missile")
        ...
        data.addScript {
            ...
            Global.getSector().memoryWithoutUpdate.set("\$os_nexusStart", true)
            val cargo = Global.getSector().playerFleet.cargo

            data.person.stats.addPoints(1)
            Global.getSector().playerPerson.stats.setSkillLevel("os_paradeigma", 1f)
           ...
           // val market: MarketAPI = Global.getFactory().createMarket("os_nexusStorage", "Nexus Global Storage", 0)
            Misc.setAbandonedStationMarket("os_nexusStorage", station)
            station.sensorProfile = 0f
            station.setInteractionImage("icons", "omegaflag")
...
} fun addNexusCargo(nexus: SectorEntityToken): CargoAPI { // add cargo to all nexus, undamaged ones get more stuff
    var mult = 1f
