...
class os_nexusCustomProduction (var dialog: InteractionDialogAPI): CustomProductionPickerDelegate {
    override fun getAvailableShipHulls(): MutableSet<String> {
        return Global.getSettings().allShipHullSpecs.filter { (Global.getSector().getFaction(Factions.OMEGA).knowsShip(it.hullId)) && !it.hullId.contains("station", true) }.map { it.hullId }.toMutableSet()
    }

    override fun getAvailableWeapons(): MutableSet<String> {
        return Global.getSettings().allWeaponSpecs.filter { Global.getSector().getFaction(Factions.OMEGA).knowsWeapon(it.weaponId) }.map { it.weaponId }.toMutableSet()
    }

    override fun getAvailableFighters(): MutableSet<String> {
        return Global.getSettings().allFighterWingSpecs.filter { Global.getSector().getFaction(Factions.OMEGA).knowsFighter(it.id) }.map { it.id }.toMutableSet()
        ...
        for (guy in currentstuff){
            if (guy.type == FactionProductionAPI.ProductionItemType.SHIP){
                for (i in 0 until guy.quantity){
                    ...
                    member.fixVariant()
                    member.shipName = Global.getSector().getFaction(Factions.OMEGA).pickRandomShipName()
