package data.omegastart.econ.items
...
class os_packedShip: BaseSpecialItemPlugin() {
    ...
    override fun init(stack: CargoStackAPI?) {
        ...
        Global.getSector().getEntityById("os_nexusStorage").market.getSubmarket(Submarkets.SUBMARKET_STORAGE).cargo.mothballedShips.membersListCopy.forEach {
            if (it.id == memberID){
                member = it
            }
        }
