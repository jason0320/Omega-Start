package data.omegastart.skills
...
class os_paradeigma: BaseSkillEffectDescription(), ShipSkillEffect {
    val remmy = Global.getSettings().getFactionSpec(Factions.OMEGA)
    val color = Color(100,250,210,250)
    override fun createCustomDescription(stats: MutableCharacterStatsAPI?, skill: SkillSpecAPI?, info: TooltipMakerAPI?, width: Float) {
      val para1 =  info!!.addPara("A neural link taps into the Omega mass mind.", 0f)
        para1.setHighlight("Omega")
        para1.setHighlightColor(remmy.brightUIColor)
      val para2 =  info.addPara("While you are in good relations with the Omega, confers the following effects:", 0f)
        para2.setHighlight("good", "Omega")
    ...
    override fun apply(stats: MutableShipStatsAPI?, hullSize: ShipAPI.HullSize?, id: String?, level: Float) {
        ...
        if (Global.getSector().getFaction(Factions.OMEGA).relToPlayer.rel < -0.2f){
    ...
    override fun advance(amount: Float) {
        ...
        if ( !player.stats.hasSkill("os_paradeigma") || Global.getSector().getFaction(Factions.OMEGA).relToPlayer.rel < -0.2f){
            removeCore(cargo)
    ...
    private fun addCore(cargo: CargoAPI) {
        if (cargo.getCommodityQuantity("os_playercore") == 0f) {
            cargo.addCommodity("os_playercore", 1f)
        }
    }

    private fun removeCore(cargo: CargoAPI) {
       cargo.stacksCopy.forEach { stack ->
            if (stack.isCommodityStack && stack.commodityId == "os_playercore") {
                val amt = stack.size
                cargo.removeCommodity("os_playercore", amt)
            }
        }
    }
